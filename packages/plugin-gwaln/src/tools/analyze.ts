...
export const analyzeHandler = async (
  ...
  try {
    ...
    const structuredContent = { topicId: input.topicId ?? null, results };
    return {
      content: textContent(
    ...
    const errorMessage =
      error instanceof Error
        ...
        : typeof error === "string"
          ? error
          : "Unknown error occurred while analyzing content";
    throw new Error(`[analyse] Failed to analyze content: ${errorMessage}`);
  }
};
