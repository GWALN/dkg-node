...
export const LookupInputSchema = z
  ...
  .refine(
    (value) => {
      if (value.action === "add") {
        return Boolean(
          value.title && value.wikipediaSlug && value.grokipediaSlug
        );
      }
      return true;
    },
    {
      message:
        'When action is "add", title, wikipediaSlug, and grokipediaSlug are required.',
      path: ["action"],
    }
  );
...
export const lookupHandler = async (
  ...
  try {
    ...
    if (result.searchResults) {
      ...
      return {
        content: textContent(
          `${hasResults ? `[lookup] Found ${grokipedia.length} Grokipedia and ${wikipedia.length} Wikipedia results for "${query}"` : `[lookup] No results found for "${query}"`}\n\nResults:\n${JSON.stringify(structuredContent, null, 2)}`
        ),
      };
    }
    ...
    return {
      content: textContent(
        `[lookup] Topic "${query}" not found in local catalog\n\nResults:\n${JSON.stringify(structuredContent, null, 2)}`
      ),
    };
  } catch (error) {
    const errorMessage =
      error instanceof Error
        ...
        : typeof error === "string"
          ? error
          : "Unknown error occurred while looking up topic";
    throw new Error(`[lookup] Failed to lookup topic: ${errorMessage}`);
  }
};
